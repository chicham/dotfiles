    ...
    if command -v brew > /dev/null 2>&1; then
      ...
      if command -v pkgutil > /dev/null 2>&1; then
        ...
        if [ -f temp-pkg/op.pkg/Payload ]; then
          ...
          # Ensure the directory structure exists in the temp dir
          mkdir -p "${TEMP_DIR}/usr/local/bin"
          # Check if the file was extracted to the expected path
          if [ -f "$TEMP_DIR/usr/local/bin/op" ]; then
            mv "$TEMP_DIR/usr/local/bin/op" "${HOME}/.local/bin/"
          else
            find "$TEMP_DIR" -name "op" -exec mv {} "${HOME}/.local/bin/" \;
          fi
