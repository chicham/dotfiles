...
bind('v', '<', '<gv')
bind('v', '>', '>gv')
bind('n', '<leader>pp', ":<c-u>put =join([expand('%:p'), line('.')], ':')<cr>", { silent = true })
...
bind('n', '<leader>fs', function()
  require('telescope.builtin').lsp_document_symbols { symbols = { 'class', 'method', 'function', 'module', 'variable',
    'constant' } }
end, { desc = 'Document [S]ymbols' })
-- Not supported by python-lsp-server
