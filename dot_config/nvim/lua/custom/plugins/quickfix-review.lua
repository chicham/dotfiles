return {
  ...
  config = function()
    ...
    local function write_export_file()
      ...
      if #qf_list == 0 then
        if qr_config.options.export_file then
          os.remove(qr_config.options.export_file)
        end
        return
      end
    ...
    local function parse_ref(ref)
      ...
      if file then
        return file, tonumber(lnum), tonumber(end_lnum), tonumber(col), tonumber(end_col)
      end

      file, lnum, col, end_col = ref:match("^(.*):(%d+):(%d+)%-(%d+)$")
      if file then
        return file, tonumber(lnum), tonumber(lnum), tonumber(col), tonumber(end_col)
      end

      file, lnum, end_lnum = ref:match("^(.*):(%d+)%-(%d+)$")
      if file then
        return file, tonumber(lnum), tonumber(end_lnum), nil, nil
      end

      file, lnum = ref:match("^(.*):(%d+)$")
      if file then
        return file, tonumber(lnum), tonumber(lnum), nil, nil
      end
    ...
    qr.add_comment = function(comment_type, range)
      ...
      local where = start_line == end_line and ("L" .. start_line) or ("L" .. start_line .. "-" .. end_line)
